...
use crate::models::quiz::{QuizRequestSchema, QuizResponse};
use crate::models::workspace::{Workspace, WorkspaceMember};
use crate::services::{ai_gateway::AIGateway, quiz::QuizGenerator};

pub fn router() -> Router {
    Router::new()
        .route("/{workspace_id}/quizzes", post(generate_workspace_quiz))
}
...
async fn generate_workspace_quiz(
    State(pool): State<PgPool>,
    State(settings): State<Settings>,
    State(ai_gateway): State<AIGateway>,
    auth_user: AuthUser,
    Path(workspace_id): Path<Uuid>,
    Json(payload): Json<QuizRequestSchema>,
    ...
    if !["owner", "editor"].contains(&role.as_str()) {
        return Err(AppError::Forbidden("Only workspace owners or editors can generate quizzes".to_string()));
    }

    let topic = payload.topic.unwrap_or_else(|| "Molecular Chemistry & Bonding".to_string());
    let num_questions = payload.num_questions.unwrap_or(3).clamp(1, 10);

    let quiz_generator = QuizGenerator::new();
    let request = crate::models::quiz::QuizGenerationRequestInternal {
        workspace_id,
        topic,
        num_questions,
    ...
    let result = quiz_generator.generate_quiz(&ai_gateway, request).await?;
    Ok(Json(result))
}