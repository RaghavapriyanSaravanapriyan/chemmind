...
import {
  ...
  FolderOpen, ArrowUpRight, Sparkles, X, ChevronRight,
} from "lucide-react";
...
const INITIAL_PROJECTS: Project[] = [
  ...
  { id: "bio-chem", name: "Biochemistry", description: "Protein structures and enzyme kinetics", docs: 6, lastOpened: "Last week", color: "from-rose-500 to-pink-600" },
];
...
export default function ProjectsPage() {
  ...
  const handleCreate = () => {
    ...
    const id = newName.toLowerCase().replace(/\s+/g, "-").slice(0, 20) + "-" + Date.now();
    setProjects([
      { id, name: newName.trim(), description: newDesc.trim() || "No description", docs: 0, lastOpened: "Just now", color: newColor },
      ...projects,
    ]);
    setNewName("");
    setNewDesc("");
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease }}>
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-muted">Your workspace</p>
          <h1 className="mt-2 text-3xl font-bold tracking-[-0.04em] sm:text-4xl">Projects</h1>
          <p className="mt-2 text-base text-muted">Pick up where you left off, or start something new.</p>
        </motion.div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-bold tracking-[-0.03em]">New Project</h2>
                  <p className="mt-1 text-sm text-muted">Create a workspace for your study material.</p>
                </div>
                <button onClick={() => setShowCreate(false)} className="flex size-8 items-center justify-center rounded-lg text-muted hover:bg-background hover:text-foreground transition-colors">
                  <X size={18} />
                </button>
              </div>
              ...
              <input
                ...
                type="text" placeholder="e.g. Organic Chemistry"
                autoFocus
              ...
              <label className="text-xs font-semibold text-muted block mb-1.5 mt-4">Description (optional)</label>
              <textarea
                value={newDesc} onChange={(e) => setNewDesc(e.target.value)}
                rows={2} placeholder="What will you study?"
                ...
                {COLOR_OPTIONS.map((c) => (
                  <button
                    ...
                    className={cn(
                      ...
                      c,
                      newColor === c ? "ring-2 ring-foreground ring-offset-2 ring-offset-surface scale-110" : "hover:scale-110"
                    )}
                  />
                ))}
              </div>
              ...
              <div className="mt-7 flex gap-3">
                <button onClick={() => setShowCreate(false)} className="flex-1 rounded-xl border border-border/70 px-4 py-3 text-sm font-medium text-muted transition-colors hover:text-foreground hover:border-border">
                  Cancel
                </button>
                <button
                  onClick={handleCreate}
                  disabled={!newName.trim()}
                  className="flex-1 rounded-xl bg-foreground px-4 py-3 text-sm font-semibold text-background transition-all hover:-translate-y-px hover:shadow-lg disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:translate-y-0"
                >
                  Create Project
                </button>
              </div>
...
function ProjectCard({ project, menuOpen, onMenuToggle, onDelete }: {
  project: Project; menuOpen: boolean; onMenuToggle: () => void; onDelete: () => void;
}) {
  return (
    <div className="group relative rounded-2xl border border-border/50 bg-surface transition-all duration-400 hover:-translate-y-1 hover:border-border hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)]">
      {/* Color bar */}
          ...
          <div className="relative">
            ...
            <AnimatePresence>
              {menuOpen && (
                ...
                >
                  <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs text-muted hover:bg-background hover:text-foreground transition-colors">
